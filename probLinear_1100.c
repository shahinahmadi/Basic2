...
{
  TFile *tmp = new TFile("LinearProb_1100.root", "recreate");
  tmp->cd();
  double dcp_in =  0.;
  double dcp_in1 =  90.;
  double dcp_in2 =  180.;
...
/// Oscillation Parameters
  ...
  double DM2_no     =  h_no * 2.4e-3;
  double Theta23 =  0.5    ;
  double Theta13 =  0.0256 ;
  double dm2     =  7.6e-5;
  double Theta12 =  0.312;
  double delta   =  dcp_in * (3.1415926/180.0);
  double delta1   =  dcp_in1 * (3.1415926/180.0);
  double delta2   =  dcp_in2 * (3.1415926/180.0);
  double delta3   =  dcp_in3 * (3.1415926/180.0);
 ...
 ///////////////////////////
  ...
  TH1D * lmu2eE_delta1 = new TH1D("lmu2eL_Al_delta1", ssE.str().c_str() , NBinsEnergy  -1 ,EnergyBins  );
  TH1D * lmu2eE_delta2 = new TH1D("lmu2eL_Al_delta2", ssE.str().c_str() , NBinsEnergy  -1 , EnergyBins  );
  TH1D * lmu2eE_delta3 = new TH1D("lmu2eL_Al_delta3", ssE.str().c_str() , NBinsEnergy  -1 , EnergyBins  );
TH1D * lmu2eE_h_no = new TH1D("lmu2eL_h_no", ssE.str().c_str() , NBinsEnergy  -1 , EnergyBins  );

  // Density
  ssE.str(""); ssE <<  "P(#nu_{e} #rightarrow #nu_{e})" << " L = " << BasePath ; 
  ssE <<  " E = " << BaseEnergy; 
  TH1D * lmu2E_Density = new TH1D("lmu2E_Density", ssE.str().c_str() , NBinsDensity  -1 , DensityEdge );
 TH1D * lmu2E_Density1  = new TH1D("lmu2E_Density1", ssE.str().c_str() , NBinsDensity  -1 , DensityEdge );
  ...
  ///////////////////
  //for normal heirarchy
for ( i = 0 ; i <= NBinsEnergy ; i ++ ) 
  {
     energy = e_start*pow(10.0, double(i)*e_step);
     bNu->SetMNS( Theta12,  Theta13, Theta23, dm2, DM2_no, delta , energy, kSquared, kNuBar ); 
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     ...
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     lmu2eE_h_no->Fill( energy, bNu->GetProb(2,1) );
     
  }


  //////////////////////
  //for plotting delta CP at 90 degrees
 for ( i = 0 ; i <= NBinsEnergy ; i ++ ) 
  {
     energy = e_start*pow(10.0, double(i)*e_step);
     bNu->SetMNS( Theta12,  Theta13, Theta23, dm2, DM2, delta1 , energy, kSquared, kNuBar ); 
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     ...
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     lmu2eE_delta1->Fill( energy, bNu->GetProb(2,1) );
  }


 //////////////////////
  //for plotting delta CP at 180 degrees
 for ( i = 0 ; i <= NBinsEnergy ; i ++ ) 
  {
     energy = e_start*pow(10.0, double(i)*e_step);
     bNu->SetMNS( Theta12,  Theta13, Theta23, dm2, DM2, delta2 , energy, kSquared, kNuBar ); 
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     ...
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     lmu2eE_delta2->Fill( energy, bNu->GetProb(2,1) );
  }

 //////////////////////
  //for plotting delta CP at 270 degrees
 for ( i = 0 ; i <= NBinsEnergy ; i ++ ) 
  {
     energy = e_start*pow(10.0, double(i)*e_step);
     bNu->SetMNS( Theta12,  Theta13, Theta23, dm2, DM2, delta3 , energy, kSquared, kNuBar ); 
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     ...
     bNu->propagateLinear( 1*kNuBar, BasePath, Density );
     lmu2eE_delta3->Fill( energy, bNu->GetProb(2,1) );
  }

 ///////////////////
 //for path legth distribution
  ...
  for ( i = 0 ; i <= NBinsDensity ; i ++ ) 
  {
     ...
     lmu2E_Density->Fill( Density , bNu->GetProb(2,1) );  
  }


  //////////////////////
  //for density 0.0g/cm^3
for ( i = 0 ; i <= NBinsDensity ; i ++ ) 
  {
     Density = d_start +  double(i)*d_step ; 
     bNu->SetMNS( Theta12,  Theta13, Theta23, dm2, DM2, delta , BaseEnergy, kSquared, kNuBar ); 
     bNu->propagateLinear( 1*kNuBar, BasePath , Density1 );
     lmu2E_Density1->Fill( Density , bNu->GetProb(2,1) );  
  }
