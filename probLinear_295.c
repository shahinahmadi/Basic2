...
{
  TFile *tmp = new TFile("LinearProb_295.root", "recreate");
  tmp->cd();
  double dcp_in =  0.;
  double dcp_in1 =  90.;
  double dcp_in2 =  180.;
...
/// Oscillation Parameters
 ...
 ///////////////////////////
  ...
  TH1D * lmu2eE_delta1 = new TH1D("lmu2eL_delta1", ssE.str().c_str() , NBinsEnergy  -1 ,EnergyBins  );
  TH1D * lmu2eE_delta2 = new TH1D("lmu2eL_delta2", ssE.str().c_str() , NBinsEnergy  -1 , EnergyBins  );
  TH1D * lmu2eE_delta3 = new TH1D("lmu2eL_delta3", ssE.str().c_str() , NBinsEnergy  -1 , EnergyBins  );
TH1D * lmu2eE_h_no = new TH1D("lmu2eL_h_no", ssE.str().c_str() , NBinsEnergy  -1 , EnergyBins  );

  // Density
  ssE.str(""); ssE <<  "P(#nu_{e} #rightarrow #nu_{e})" << " L = " << BasePath ; 
  ssE <<  " E = " << BaseEnergy; 
  TH1D * lmu2E_Density = new TH1D("Density", ssE.str().c_str() , NBinsDensity  -1 , DensityEdge );
 TH1D * lmu2E_Density1  = new TH1D("Density1", ssE.str().c_str() , NBinsDensity  -1 , DensityEdge );
